...
if (!jobIdArg || !/^\d+$/.test(jobIdArg)) {
  throw new Error(
    "Provide a numeric job ID, for example: npx tsx .\\scripts\\refund-expired-job.ts 3",
  );
}
...
async function main() {
  ...
  console.log("Expired-job refund initiated:", response.data);
  ...
  if (currentState !== "COMPLETE") {
    throw new Error(
      "Expired-job refund ended in state: " + currentState,
    );
  }
}

main().catch((error) => {
  console.error("Expired-job refund failed:", error);
  process.exit(1);
});