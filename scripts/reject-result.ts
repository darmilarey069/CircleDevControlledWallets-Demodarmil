...
if (!jobIdArg || !/^\d+$/.test(jobIdArg)) {
  throw new Error(
    "Provide a numeric job ID, for example: npx tsx .\\scripts\\reject-result.ts 2",
  );
}
...
async function main() {
  const response =
    await client.createContractExecutionTransaction({
      ...
      abiFunctionSignature: "rejectResult(uint256)",
  ...
  console.log("Rejection initiated:", response.data);
  ...
  if (currentState !== "COMPLETE") {
    throw new Error(
      "Rejection ended in state: " + currentState,
    );
  }
}
